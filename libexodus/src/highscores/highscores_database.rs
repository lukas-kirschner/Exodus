...
impl HighscoresDatabase {
    ...
    pub fn put(&mut self, map: [u8; 32], player: String, timestamp: i64, highscore: Highscore) -> &mut Self {
        self.records.entry(map)
            .or_insert(HighscoreRecords::new(map.clone()))
    ...
    pub fn put_with_current_time(&mut self, map: [u8; 32], player: String, highscore: Highscore) -> &mut Self {
        self.records.entry(map)
            .or_insert(HighscoreRecords::new(map.clone()))
...
mod tests {
    ...
    fn test_multiple_highscores() {
        let mut database = HighscoresDatabase::new();
        database.put([0u8; 32], "Thorsten".to_string(), 1337, Highscore::new(10, 0));
        assert_eq!(1, database.len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().get(&"Thorsten".to_string()).unwrap().len());

        database.put([0u8; 32], "Thorsten".to_string(), 69, Highscore::new(10, 0));
        assert_eq!(1, database.len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().get(&"Thorsten".to_string()).unwrap().len());

        database.put([0u8; 32], "Frank".to_string(), 1337, Highscore::new(10, 0));
        assert_eq!(1, database.len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().get(&"Thorsten".to_string()).unwrap().len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().get(&"Frank".to_string()).unwrap().len());

        database.put([1u8; 32], "Thorsten".to_string(), 1337, Highscore::new(10, 0));
        assert_eq!(2, database.len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().get(&"Thorsten".to_string()).unwrap().len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().get(&"Frank".to_string()).unwrap().len());
        assert_eq!(1, database.get(&[1u8; 32]).unwrap().len());
        assert_eq!(1, database.get(&[1u8; 32]).unwrap().get(&"Thorsten".to_string()).unwrap().len());

        let (time, best1) = database.get_best(&[0u8; 32], &"Thorsten".to_string()).unwrap();
        assert_eq!(1337, time);
    ...
    pub fn test_put_with_current_time() {
        ...
        assert_eq!(1, database.len());
        assert_eq!(1, database.get(&[0u8; 32]).unwrap().len());
        assert_eq!(2, database.get(&[0u8; 32]).unwrap().get(&"Leo".to_string()).unwrap().len());
